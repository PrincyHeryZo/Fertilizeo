...
const Notifications: React.FC = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Marquer toutes comme lues automatiquement dès ouverture de la page
      const unread = response.data.filter((n: any) => !n.is_read);
      if (unread.length > 0) {
        await api.put('/notifications/read-all');
        setNotifications(response.data.map((n: any) => ({ ...n, is_read: true })));
        // Signaler à la Navbar de remettre le badge à 0
