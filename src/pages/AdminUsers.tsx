    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-3">
          {[1,2,3,4,5].map(i => <div key={i} className="h-16 bg-white rounded-2xl animate-pulse border border-gray-100" />)}
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((u) => (
                <motion.tr key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <td className="p-4 text-gray-600 text-sm hidden md:table-cell">{u.location || '—'}</td>
                  <td className="p-4 text-gray-500 text-sm hidden lg:table-cell">{new Date(u.created_at).toLocaleDateString('fr-FR')}</td>
                  <td className="p-4">
                    <div className="relative inline-block">
                      <select value={u.role} disabled={updatingId === u.id}
                        onChange={e => handleRoleChange(u.id, e.target.value)}
                        className={`appearance-none pl-3 pr-8 py-1.5 rounded-full text-xs font-bold cursor-pointer outline-none ${roleColors[u.role] || roleColors['Acheteur']} ${updatingId === u.id ? 'opacity-50' : ''}`}>
