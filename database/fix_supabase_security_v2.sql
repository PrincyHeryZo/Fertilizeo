-- ============================================================
-- FERTILI'ZEO - Correction des vulnérabilités de sécurité Supabase (V2)
-- Version corrigée pour gérer les types de données correctement
...
-- Users: Utiliser une colonne email pour matcher avec auth.uid() car id est integer
...
CREATE POLICY "Users can update own posts" ON forum_posts FOR UPDATE USING (
  EXISTS (SELECT 1 FROM users WHERE users.email = auth.email() AND users.id = forum_posts.user_id)
);
...
CREATE POLICY "Users can update own comments" ON forum_comments FOR UPDATE USING (
  EXISTS (SELECT 1 FROM users WHERE users.email = auth.email() AND users.id = forum_comments.user_id)
);
