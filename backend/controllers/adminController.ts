...
export const getPendingProducts = async (req: Request, res: Response) => {
    try {
        const products = await db.all(
            'SELECT p.*, u.name as producer_name, u.location as producer_location FROM products p JOIN users u ON p.producer_id = u.id WHERE p.is_approved = FALSE'
        );
        res.json(products);
    } catch (error) {
...
export const approveProduct = async (req: Request, res: Response) => {
    try {
        await db.run('UPDATE products SET is_approved = TRUE WHERE id = ?', [req.params.id]);
